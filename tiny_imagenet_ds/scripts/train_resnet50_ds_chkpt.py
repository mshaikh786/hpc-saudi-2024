...
def add_argument():
    ...
    parser.add_argument(
        "--resume",
        action='store_true',
        help="start from an existing checkpoint",
    )
    parser.add_argument(
        "--log-interval",
        type=int,
...
def main(args):
    ...
    if args.resume : 
       path,client_state=model_engine.load_checkpoint(args.checkpoint_dir)
   ...
   ########################################################################
    ...
    if ( (args.resume is True) and (global_rank == 0)):
        print(f'load_path={path}, client_state={client_state.keys()}')
    ...
    for epoch in range(start_epoch,args.epochs):  # loop over the dataset multiple times
        ...
        model_engine.save_checkpoint(save_dir=args.checkpoint_dir, 
                                     tag=epoch,
                                     client_state={'checkpoint_step': i,
                                                   'epoch': epoch},
                                     )
